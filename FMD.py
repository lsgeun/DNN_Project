...
class FMD():
    ...
    def show_efficiency(self, metric_name='FM_mean rmw_max,1000 all se_lfmd C rvalid_fmds_average', eval_name='test', show_category=True):
        ...
        index = np.array([0])
        label = [f'{class_name}']
        p1 = plt.bar(index - (bar_width)/2, self.metrics[metric_name]['N'][eval_name], bar_width, color='blue', alpha=alpha, label='U efficiency')
        p2 = plt.bar(index + (bar_width)/2, self.metrics[metric_name]['NPV'][eval_name], bar_width, color='orange', alpha=alpha, label='FMD efficiency')
